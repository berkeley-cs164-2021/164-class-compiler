...
let rec interp_exp (defns: defn list) (env : value symtab) (exp : s_exp) : value =
  ...
  | Lst [Sym "num?"; arg] -> (
    match interp_exp defns env arg with Number _ -> Boolean true | _ -> Boolean false )
  | Lst [Sym "if"; test_exp; then_exp; else_exp] ->
    if interp_exp defns env test_exp <> Boolean false then interp_exp defns env then_exp
